...
StreamParser::State StreamParser::handle_column_enclose_char() noexcept {
  ...
  if (m_column_end == nullptr) {
    ...
    m_row.m_column_ptrs[m_column_idx] = m_column_begin;
    m_row.m_column_lens[m_column_idx] = m_column_end - m_column_begin;
    return (m_column_idx < m_row.m_num_columns)
               ? State::GetColumnStart : State::GetRowEnd;
  }
