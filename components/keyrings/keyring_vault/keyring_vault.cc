...
bool init_or_reinit_keyring() {
  ...
  auto new_backend =
      std::make_unique<Keyring_vault_backend>(*new_config_pod.get());
  if (!new_backend || !new_backend.get()->valid()) return true;

  /* Create new operations class */
  auto new_operations = std::make_unique<Keyring_operations<Keyring_vault_backend>>(true, new_backend.release());
