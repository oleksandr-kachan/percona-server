...
#include <components/keyrings/keyring_vault/keyring_vault.h> /* Globals */
...
using keyring_vault::g_component_callbacks;
using keyring_vault::g_keyring_operations;
...
DEFINE_BOOL_METHOD(Keyring_keys_metadata_iterator_service_impl::init,
  ...
  bool retval = init_keys_metadata_iterator_template<Keyring_vault_backend>(
...
return deinit_keys_metadata_iterator_template<Keyring_vault_backend>(
   it, *g_keyring_operations, *g_component_callbacks);
}
...
std::unique_ptr<Iterator<Data>> it;
it.reset(reinterpret_cast<Iterator<Data> *>(forward_iterator));
bool retval = keys_metadata_iterator_is_valid<Keyring_vault_backend>(
   it, *g_keyring_operations, *g_component_callbacks);
...
std::unique_ptr<Iterator<Data>> it;
it.reset(reinterpret_cast<Iterator<Data> *>(forward_iterator));
bool retval = keys_metadata_iterator_next<Keyring_vault_backend>(
   it, *g_keyring_operations, *g_component_callbacks);
/* Make sure we don't free the pointer */
(void)it.release();
return retval;
...
std::unique_ptr<Iterator<Data>> it;
it.reset(reinterpret_cast<Iterator<Data> *>(forward_iterator));
bool retval = keys_metadata_get_length_template<Keyring_vault_backend>(
...
std::unique_ptr<Iterator<Data>> it;
it.reset(reinterpret_cast<Iterator<Data> *>(forward_iterator));
bool retval = keys_metadata_get_template<Keyring_vault_backend>(
